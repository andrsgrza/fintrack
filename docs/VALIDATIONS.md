...
| **Tests** | `FinancialAccountResourceIT` (ownership, immutability, timestamp hardening, delete orchestration, initial date floor); `FinancialAccountServiceTest` (ownership, immutability, timestamp hardening, orchestration, floor unit); frontend opening-position spec verifies labels plus hidden timestamps / locked immutable selects. PATCH IT uses minimal JSON — see [TESTING.md § FA](TESTING.md#financialaccount). |
