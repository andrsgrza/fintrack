...
| **REST/UI** | `@Valid` POST/PUT; PATCH **JsonNode** (`currency`/`accountType`/timestamps null → `400`); `isAccessible()` on mutate; `400 invalid` on business errors. UI does not expose timestamp inputs; edit mode locks `currency` and `accountType`. |
| **Tests** | `FinancialAccountResourceIT` (ownership, immutability, timestamp hardening, initialBalance monetary scale, delete orchestration, initial date floor); `FinancialAccountServiceTest` (ownership, immutability, timestamp hardening, initialBalance monetary scale, orchestration, floor unit); frontend opening-position spec verifies labels plus hidden timestamps / locked immutable selects. PATCH IT uses minimal JSON — see [TESTING.md § FA](TESTING.md#financialaccount). |
