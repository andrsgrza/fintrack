...
- The single review page has a local, non-persisted automation configuration: auto-apply descriptions, categories, and tags default off; protect manual changes defaults on. With a scope off, its scoped reevaluation remains a preview. With it on, reevaluation also applies the current result to that scope. Suggestions are accepted by domain: the Description cell offers **Apply suggested description**, the Category cell **Apply suggested category**, and the Tags cell **Apply suggested tags**. The always-visible Apply group separately exposes the matching three batch commands. Category/tag applications use `scope=CATEGORY` or `scope=TAGS`, respectively, preserve the other domain's transient preview, and retain `FILL_EMPTY_ONLY` semantics. Description actions preserve classification suggestions. The single-row description action is explicit consent and can replace a `USER_EDIT`; batch description application honors manual protection. Automatic category/tag application is candidate-backed and never writes category/tags to `rawData`; automatic description application uses the existing normalized-description/review model. No review action creates a `FinancialTransaction`.
...
No FinancialTransactions are created during upload/review. Candidate preparation also creates no FinancialTransactions and leaves `rawData` unchanged. Workflow GET exposes prepared candidates but remains read-only, including read-only category/tag provenance (`categorySource` and per-tag source). The unified review uses candidate-backed classification commands that persist category/tags on prepared `FILE_IMPORT` candidates, never in `rawData`. Local automatic category/tag scopes synchronize `AUTOMATIC` values to the current rule suggestions; protected `MANUAL` category/tag values remain, while unprotected automation may replace/remove them. Explicit category and tag application remains domain-scoped `FILL_EMPTY_ONLY`, both per row and globally; applying one scope retains the other scope's transient suggestion. The same local configuration can request description preview or apply through the existing description provenance model. Confirm Import posts category/tags into final `FinancialTransaction` rows but does not copy provenance. The frontend validates persisted candidates, then posts `/confirm` with no legacy `records`/category/tag payload; backend category/tags come from persisted candidates. The old `classification-preview` implementation and backend parsing of the old confirm `records/categoryId/tagIds` body are removed. UserPreference-driven rule behavior is deferred.

### Inactive account after workflow creation (ACC-UX-3A)

Creating a new FILE workflow requires an active, current-owner account, and the new-ingestion selector lists only those accounts. If that already-linked account is deactivated later, the workflow remains accessible: candidate preparation, review, reevaluation, suggestion application, and Confirm Import continue using the historical reference. Confirm Import intentionally does not revalidate `FinancialAccount.active`, and no account/category/tag data is added to `rawData`.
