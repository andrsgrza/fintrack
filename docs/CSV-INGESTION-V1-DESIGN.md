...
- The single review page has a local, non-persisted automation configuration: auto-apply descriptions, categories, and tags default off; protect manual changes defaults on. With a scope off, its scoped reevaluation remains a preview. With it on, reevaluation also applies the current result to that scope. A previewed description can instead be accepted manually with **Apply suggested description** for one row, or **Apply all suggested descriptions** for the batch of rows that currently carry a transient recommendation. The single-row action is explicit consent and can replace a `USER_EDIT`; batch application honors manual protection. Automatic category/tag application is candidate-backed and never writes category/tags to `rawData`; automatic description application uses the existing normalized-description/review model. No review action creates a `FinancialTransaction`. The separate **Apply all classification suggestions** action remains the conservative category/tag `FILL_EMPTY_ONLY` command with no `candidateIds`; it never applies description normalization suggestions and does not change semantics based on the configuration.
