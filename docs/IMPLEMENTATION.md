...
TC-5E.4 centralizes the common candidate-to-`FinancialTransaction` field copy in the internal `TransactionCandidateFinancialTransactionMapper`. Source-specific services still own lifecycle validation, idempotency, source/origin choice, status transitions, transaction boundaries, and links such as FILE import `transactionIngestion`.

Generic `TransactionCandidate` CRUD write endpoints remain technical/restricted compatibility surfaces. They can create only basic unlinked `MANUAL` drafts. They cannot create `FILE_IMPORT`/`API_IMPORT` candidates, cannot set ingestion links or `financialTransaction`, cannot directly change lifecycle status, cannot set lifecycle/review/server-controlled fields, cannot write derived `amount`/`flow`, and preserve source immutability. Generic update/PATCH is rejected for `FILE_IMPORT`, `API_IMPORT`, and workflow-linked candidates because those are owned by workflow command endpoints. Generic delete is limited to unlinked `MANUAL` `DRAFT` candidates; it rejects `FILE_IMPORT`, `API_IMPORT`, `POSTED`, `CANCELLED`, workflow-linked, and posted-transaction-linked candidates. Manual lifecycle changes must go through the manual command endpoints.

Direct `FinancialTransaction` delete is also candidate-aware. If any `TransactionCandidate` links to the transaction, direct delete returns `400 invalid` before mutating ingestion records, internal transfers, tag joins, or the transaction itself. `POSTED` candidates use the clearer provenance message (“posted from a transaction candidate”); non-`POSTED` linked candidates are treated as corrupt/invalid state and also block direct delete. TransactionIngestion workflow delete is the exception because it owns the whole import tree and removes candidate tag joins/candidates first, then imported transactions in FK-safe order.
...
- `POST /api/transaction-ingestions/{ingestionId}/candidates/rule-preview` evaluates current candidate state read-only using `TransactionOrigin.FILE_IMPORT`. Its optional `scope=CATEGORY|TAGS|ALL` filters the transient output without applying category/tags.
...
- When a `READY` workflow has valid rows without candidates, the same review page calls `POST /api/transaction-ingestions/{id}/candidates/prepare` once for that missing-row set, reloads `GET /api/transaction-ingestions/{id}/workflow`, and requires every `VALID` row to have a usable `FILE_IMPORT` candidate before category/tag actions are enabled.
- The row `candidate` summary is the source of truth for selected category, tags, and classification review status; it is rendered beside the normalized row fields rather than in a separate stage.
- Batch preview calls `POST /api/transaction-ingestions/{id}/candidates/rule-preview` and keeps suggestions transiently keyed by candidate id. The single review page can scope this preview to categories, tags, or both; scoped results merge into the corresponding transient UI state without changing candidate selections.
- `POST /api/transaction-ingestions/{id}/descriptions/reevaluate` reevaluates all eligible rows or an optional record-id subset with the existing description evaluator. Its request can be preview-only or apply, and defaults to protecting a `USER_EDIT` value. The review UI uses it for a transient preview, explicit per-row acceptance (`apply=true, protectManualChanges=false`), and batch acceptance of currently suggested rows (`apply=true` with the local protection setting). A server-authoritative apply reevaluates immutable source text and the UI reloads the resulting persisted description/provenance.
- The same one-screen review embeds reusable configured rule forms. `POST /api/description-normalization-rules/configured` atomically creates a description rule plus ordered conditions; TransactionRule uses its existing configured command API. Global actions have no ingestion prefill. Row description-rule actions prefill immutable original description plus the current normalized/manual result; row TransactionRule actions prefill persisted candidate description, flow, and persisted category/tag outputs only.
- A rule save is isolated from review state. It does not reevaluate, apply, confirm, create transactions, or persist category/tags into `rawData`. Contextual **Save + reevaluate this row/all** actions explicitly invoke the existing description or candidate classification reevaluation paths after the rule command succeeds, using the page-local automation configuration.
...
- Direct FinancialTransaction delete rejects transactions linked to `TransactionCandidate` provenance. TransactionIngestion workflow delete may still remove the whole workflow tree, including posted `FILE_IMPORT` candidates and their imported transactions, through its controlled cleanup path.
- Category and Tag direct delete reject if any `TransactionCandidate` references the category/tag; candidate review selections are not silently unlinked. FinancialAccount direct delete rejects manual/non-workflow candidates and then verifies controlled ingestion cleanup removed any account-linked `FILE_IMPORT` candidates before deleting the account.
- CSV ingestion uses `IngestionRecord.rawData.normalized` for row transaction review while category/tag decisions persist on `FILE_IMPORT` candidates in the same unified review.
...
- Candidate-to-`FinancialTransaction` base field mapping is shared internally; Confirm Import still adds the FILE-specific `transactionIngestion` link and owns record/candidate/parent status transitions.
- No UserPreference, persisted reevaluation preference, draft dashboard/menu entry, or CSV/API TransactionCandidate generic product UI exists yet. The TransactionIngestion configuration is deliberately local to its review page.
...
- TC-5E.3 centralizes candidate `FILL_EMPTY_ONLY` mutation in the internal `TransactionCandidateRuleApplicationService`. Manual and FILE_IMPORT services still own authorization, source/status guards, rule evaluation, save timing, response mapping, and workflow-specific errors.
...
**Product overview:** `GET /api/financial-accounts/overview` returns a read-only `FinancialAccountOverviewDTO` for every account visible to the current user. It batches the balance inputs server-side rather than making one browser request per account. The response contains product identity, the persisted account color for visual accenting, calculated balance/debt, and useful card details only; it persists no balance. The Financial Accounts list uses this endpoint and does not expose generated ids, audit fields, raw color text, or relationship columns.
...
**ACC-POLISH — account composition:** This is a presentation-only refinement except for the read-only color projection above. The list uses account cards with color accents, compact Edit, and a More menu for Delete. The canonical detail uses an identity header and type-specific balance hierarchy; the account form is sectioned and hides the unbacked icon field. Existing configured write endpoints, account formulas, status policy, routes, and atomic card persistence remain unchanged.
...
| `CREDIT_CARD` product command requires details | ✅     | Configured create/update validates and persists account + details atomically |
| Cascade en FA delete                           | ✅     | `FinancialAccountService` deletes details during account delete              |

**Fuera de scope:** cálculos de interés, statement generation, and changes to generic FinancialAccount/CreditAccountDetails CRUD semantics.
...
| FILE import candidate-backed classification     | ✅     | The unified TransactionIngestion review uses candidate summaries as source of truth, persists selections on candidates, and TC-3D.2 confirm sends no legacy selection payload                                                                                                                |
| FILE import candidate-backed Confirm Import     | ✅     | TC-3D.1 backend Confirm Import posts reviewed `FILE_IMPORT` candidates as source of truth, links candidates/records/transactions, and preserves rawData; TC-4B removes old selection-body parsing; TC-5D.4 rejects corrupt pre-completed candidate/row/transaction link state all-or-nothing |
...
| Source payload      | ✅ Uses reviewed `FILE_IMPORT` `TransactionCandidate` fields/category/tags synced from `IngestionRecord.rawData.normalized` and unified-screen classification decisions.                                                                                                                                                                                                                                                                                                                           |
...
| Classification      | ✅ The unified review uses ingestion-scoped candidate preview/apply commands; TC-3D.1 Confirm Import requires reviewed candidate classification. TC-4B removes the old `classification-preview` endpoint/service/DTO path.                                                                                                                                                                                                                                                                         |
| Unified review UI   | ✅ READY workflows prepare missing candidates automatically, reload the workflow, preview suggestions transiently, and render persisted category/tag decisions beside the row review fields.                                                                                                                                                                                                                                                                                                       |
| Scoped reevaluation | ✅ The same screen exposes description, category, tag, all, and eligible-row reevaluation plus local auto-apply settings. Defaults keep descriptions/categories/tags preview-only and protect manual changes. Each suggestion is applied from its domain: description, category, and tags each have per-row and always-visible batch actions; category/tag commands are independent `FILL_EMPTY_ONLY` scopes. Neither automatic review nor explicit Apply confirms import or creates transactions. |
...
| Rule Engine         | ✅ CSV v1 confirm import does not invoke the Rule Engine itself and does not persist evaluation results or selections into `rawData`. Local review auto-apply configuration is implemented; UserPreference remains deferred.                                                                                                                                                                                                                                                                       |
| Counters            | ✅ Import counters are recalculated after confirm.                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| Delete cleanup      | ✅ Workflow delete removes candidate tag joins/candidates before imported `FinancialTransaction` rows; direct FinancialTransaction delete is blocked for candidate-linked transactions.                                                                                                                                                                                                                                                                                                            |
...
| Apply rules            | ✅ `POST /api/transaction-ingestions/{ingestionId}/candidates/apply-rules` keeps `FILL_EMPTY_ONLY` for explicit Apply actions (omitted body/`candidateIds` means all eligible candidates; an id list means per-row scope). The same command now also accepts `scope=CATEGORY                                                                                                                                      | TAGS | ALL`, `automatic=true`, and `protectManualChanges`; automatic mode synchronizes only the requested category/tag scope using server-owned provenance. `AUTOMATIC`values are replaced/cleared to match current suggestions; protected`MANUAL` values remain; unprotected automation can replace/remove them. TC-5E.3 shares the in-memory candidate mutation helper with MANUAL apply-rules while keeping FILE_IMPORT workflow guards in this service. |
...
| Guards                 | ✅ Endpoints require current-user owned FILE parent ingestion, `FILE_IMPORT` candidate source, non-final candidate status, linked `VALID` ingestion record, account presence, category/tag ownership, and category-flow compatibility. Category-flow compatibility uses the same internal enum validator as manual candidates and posted FinancialTransaction validation; workflow guards remain source-specific. |
...
| UI boundary            | ✅ The frontend uses these endpoints for unified candidate-backed category/tag review.                                                                                                                                                                                                                                                                                                                            |
...
| 2026-07-11 | CreditAccountDetails domain rules ✅: direct DELETE blocked (`400` invalid; admin no bypass; foreign `404`); mutable credit fields without utilization checks. Generic CRUD remains technically compatible.                                                                                                                                                                                                                                                                                                                                                      |
...
| 2026-09-17 | ACC-UX-2 configured FinancialAccount write ✅: product create/edit sends one parent + optional card-child request to `POST`/`PUT /api/financial-accounts[/id]/configured`; card writes are atomic, account type/currency stay immutable on update, and the UI no longer issues a second child write.                                                                                                                                                                                                                                                             |
...
| 2026-09-06 | CSV Ingestion TC-3B workflow candidate summaries ✅: `GET /api/transaction-ingestions/{id}/workflow` now exposes optional lightweight prepared `TransactionCandidate` summaries per row after TC-3A prepare; workflow GET remains read-only and does not create candidates, mutate `rawData`, or create `FinancialTransaction` rows.                                                                                                                                                                                                                             |
...
| 2026-09-08 | Historical CSV Ingestion TC-3C.2 two-step UI ✅: candidate-backed classification initially prepared FILE candidates, reloaded workflow summaries, previewed/applied/confirmed suggestions through candidate endpoints, and persisted category/tag decisions on candidates. Superseded by unified review.                                                                                                                                                                                                                                                         |
| 2026-09-08 | Historical CSV Ingestion TC-3D.2 confirm cleanup ✅: the prior two-step frontend reloaded and validated persisted candidates before calling `POST /api/transaction-ingestions/{id}/confirm` with no legacy `records`/category/tag payload. The no-body confirm contract remains current.                                                                                                                                                                                                                                                                         |
...
| 2026-09-11 | CSV Ingestion TC-5D.1 row review candidate cleanup/sync ✅: disabling removes unposted prepared `FILE_IMPORT` candidates and candidate tag joins; re-enable does not restore old candidates; edit syncs existing unposted candidates immediately when rows remain `VALID`, preserves candidate category/tags, marks fresh classification `STALE` for rule-input changes, keeps notes-only edits fresh, and removes unposted candidates when rows become non-`VALID`. Confirm Import, `rawData` shape, manual candidates, and frontend behavior remain unchanged. |
| 2026-09-11 | TransactionCandidate TC-5D.2 FinancialTransaction delete guard ✅: direct FinancialTransaction delete rejects transactions linked to MANUAL or FILE_IMPORT TransactionCandidates, including corrupt non-POSTED links, before mutating ingestion records, tags, internal transfers, or the transaction. TransactionIngestion workflow delete still cleans candidate tag joins/candidates and imported transactions in FK-safe order.                                                                                                                              |
| 2026-09-15 | TransactionCandidate TC-5D.3 reference delete guards ✅: Category/Tag delete now rejects candidate references instead of silently unlinking review selections; FinancialAccount delete rejects manual/non-workflow candidates and preserves controlled FILE_IMPORT workflow cleanup by deleting candidate joins/candidates before imported transactions/account deletion.                                                                                                                                                                                        |
| 2026-09-15 | CSV Ingestion TC-5D.4 corrupt/orphan confirm guards ✅: Confirm Import now validates all existing pre-completed `FILE_IMPORT` candidates for the ingestion, not only candidates tied to current `VALID` rows. Posted candidates must match imported rows and their `FinancialTransaction`; unposted candidates must match valid rows and have no transaction link. Corrupt state rejects all-or-nothing before new transactions are created.                                                                                                                     |
| 2026-09-15 | CSV Ingestion TI-CONFIG-1 local auto-apply configuration ✅: unified review adds local description/category/tag auto-apply controls (default off) plus manual protection (default on). Automatic description re-evaluation can preview or apply through existing provenance; automatic category/tag application synchronizes only its requested scope with server-owned provenance. Explicit Apply remains `FILL_EMPTY_ONLY`; Confirm and `rawData` behavior are unchanged.                                                                                      |
...
`POST /api/transaction-ingestions/{id}/descriptions/reevaluate` reuses this same normalization evaluator after upload. It can process all eligible rows or selected `recordIds`, always reads `rawData.raw.description`, and distinguishes preview (`apply=false`) from persisted application. The backward-compatible omitted request keeps historical apply/protect behavior. A protected `USER_EDIT` stays intact and returns a transient suggestion; **Apply suggested description** sends the explicit `apply=true, protectManualChanges=false` consent for one row, while **Apply all suggested descriptions** batches only current transient suggestions and keeps the local protection setting. The server recomputes from current ordered rules during apply and the UI reloads the persisted result/provenance. A changed non-manual normalized description is synced to an existing FILE_IMPORT candidate using the existing row/candidate sync path; reevaluation never creates a candidate, writes category/tags, or creates a FinancialTransaction.

Category/tag `TransactionRule` evaluation is not invoked during Confirm Import. Confirm Import consumes reviewed `FILE_IMPORT` candidate fields, including the candidate description that was synced from `rawData.normalized.description` during candidate preparation. The unified review's four automation controls are local UI state only (description/category/tag auto-apply off by default; manual protection on) and do not create a UserPreference. Persisted/user-level behavior remains deferred.
