...
- `POST /api/transaction-ingestions/{id}/descriptions/reevaluate` reevaluates all eligible rows or an optional record-id subset with the existing description evaluator. Its request can be preview-only or apply, and defaults to protecting a `USER_EDIT` value; only explicit unprotected application may replace that value with the current rule result.
...
| Scoped reevaluation | ✅ The same screen exposes description, category, tag, all, and eligible-row reevaluation plus local auto-apply settings. Defaults keep descriptions/categories/tags preview-only and protect manual changes. Enabled automation applies only its matching scope; explicit Apply suggestions remains `FILL_EMPTY_ONLY`. Neither automatic review nor explicit Apply confirms import or creates transactions. |
