...
| Scoped reevaluation | ✅ The same screen exposes description, category, tag, all, and eligible-row reevaluation plus local auto-apply settings. Defaults keep descriptions/categories/tags preview-only and protect manual changes. A transient description recommendation can be accepted explicitly per row or in a protection-aware batch; category/tag Apply suggestions remains separate `FILL_EMPTY_ONLY`. Neither automatic review nor explicit Apply confirms import or creates transactions. |
