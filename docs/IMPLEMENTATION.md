...
TC-2C.1b adds the frontend rule suggestions slice for manual candidates:
...
- Refresh suggestions flushes pending autosave and calls `POST /api/transaction-candidates/{id}/rule-preview`; it renders transient suggestions/conflicts/matched rules and does not mutate category/tags.
