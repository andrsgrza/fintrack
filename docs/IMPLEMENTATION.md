...
| Priority/order UX                               | ✅     | List/detail show read-only 1-based order; create/edit do not render or submit priority                                                                 |
...
| Reorder endpoint / drag-and-drop                | ⏳     | explicit user-facing priority changes deferred                                                                                                         |
...
| UI                        | ✅          | Create saves inactive parent then redirects to detail; list/detail use compact semantic summaries and read-only order; create/edit omit priority; detail/edit share identity/matching/result/status layout; edit preserves JHipster direct-field hydration; detail embeds TransactionRuleCondition editor via existing endpoints; no backend command endpoint, reorder endpoint, or create-time draft child collection yet |
