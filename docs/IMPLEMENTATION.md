...
- Confirm Import still calls the existing `POST /api/transaction-ingestions/{id}/confirm` path. Before confirm, the UI reloads the workflow and validates every `VALID` candidate is `READY_TO_POST` and classification-reviewed. The temporary frontend adapter may still send the legacy per-record payload from persisted candidate category/tag ids, but TC-3D.1 backend confirm uses persisted `FILE_IMPORT` candidates as the source of truth.
...
- Confirm Import backend internals now create `FinancialTransaction` rows from reviewed `FILE_IMPORT` candidate fields/category/tags. The legacy confirm payload is tolerated only for record coverage compatibility and does not override persisted candidates.
...
| FILE import candidate-backed Confirm Import     | ✅     | TC-3D.1 backend Confirm Import posts reviewed `FILE_IMPORT` candidates as source of truth, ignores legacy request category/tag ids, links candidates/records/transactions, and preserves rawData                |
...
| Item     | Status                                                                                                                                                                                                       |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Entry    | ✅ Canonical TransactionIngestion FILE creation workflow at `/transaction-ingestion/new`, linked from the TransactionIngestion list; FileIngestion create is parent-scoped upload, not metadata CRUD.        |
| Form     | ✅ In `/transaction-ingestion/new`, select Account + Ingestion Type and upload canonical CSV for `FILE`; API shows TBD and is not submittable. `/file-ingestion/new` selects pending FILE parent + CSV file. |
...
| Classification   | ✅ TC-3C.2 Pantalla 2 uses ingestion-scoped candidate preview/apply commands; TC-3D.1 Confirm Import requires reviewed candidate classification. The legacy `classification-preview` endpoint remains backend-compatible but is no longer the workflow UI source of truth. |
...
| 2026-09-08 | CSV Ingestion TC-3D.1 candidate-backed Confirm Import ✅: `POST /api/transaction-ingestions/{id}/confirm` now posts reviewed `FILE_IMPORT` candidates as the backend source of truth, ignores legacy request category/tag ids, links candidates and records to created `FinancialTransaction` rows, marks candidates `POSTED`, preserves `rawData`, completes all-or-nothing, and keeps completed retry idempotent.                                                |
