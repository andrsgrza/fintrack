...
- `POST /api/transaction-ingestions/{id}/descriptions/reevaluate` reevaluates all eligible rows or an optional record-id subset with the existing description evaluator. It updates normalized description/provenance only when the row is not manually edited; a `USER_EDIT` value is returned unchanged with a transient proposed result if a rule now matches.
...
- No UserPreference, reevaluation configuration/overwrite policy, draft dashboard/menu entry, or CSV/API TransactionCandidate generic product UI exists yet.
...
| Scoped reevaluation | ✅ The same screen exposes description, category, tag, all, and eligible-row reevaluation. Description reevaluation uses original raw description and preserves USER_EDIT; category/tag reevaluation uses scoped transient candidate preview. None of these commands applies selections, confirms import, or creates transactions. |
...
| Apply rules            | ✅ `POST /api/transaction-ingestions/{ingestionId}/candidates/apply-rules` re-evaluates current candidate state and applies category/tags with `FILL_EMPTY_ONLY`: category fills only when empty/no conflict; tags are additive and deduplicated; manual selections are preserved. An omitted request body/`candidateIds` is the ingestion-scoped batch contract used by **Apply all classification suggestions**; an explicit id list remains the per-row scope. TC-5E.3 shares the in-memory candidate mutation helper with MANUAL apply-rules while keeping FILE_IMPORT workflow guards in this service. |
...
`POST /api/transaction-ingestions/{id}/descriptions/reevaluate` reuses this same normalization evaluator after upload. It can process all eligible rows or selected `recordIds`, always reads `rawData.raw.description`, and does not overwrite `USER_EDIT`. A changed non-manual normalized description is synced to an existing FILE_IMPORT candidate using the existing row/candidate sync path; reevaluation never creates a candidate, writes category/tags, or creates a FinancialTransaction.

Category/tag `TransactionRule` evaluation is not invoked during Confirm Import. Confirm Import consumes reviewed `FILE_IMPORT` candidate fields, including the candidate description that was synced from `rawData.normalized.description` during candidate preparation. UserPreference-driven behavior and reevaluation configuration remain deferred.
