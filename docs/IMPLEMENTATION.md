...
| 2026-09-11 | CSV Ingestion TC-5D.1 row review candidate cleanup/sync ✅: Pantalla 1 disable removes unposted prepared `FILE_IMPORT` candidates and candidate tag joins; re-enable does not restore old candidates; edit syncs existing unposted candidates immediately when rows remain `VALID`, preserves candidate category/tags, marks fresh classification `STALE` for rule-input changes, keeps notes-only edits fresh, and removes unposted candidates when rows become non-`VALID`. Confirm Import, `rawData` shape, manual candidates, and frontend behavior remain unchanged. |
