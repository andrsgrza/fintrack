...
## TransactionCandidate — TC-1/TC-2A backend foundation + manual commands
...
TC-2A / TC-2A.1 adds backend-only manual command endpoints:
...
TC-2A does **not** change existing behavior:
...
| FinancialTransaction manual rule-preview create | ✅     | Phase 3B frontend two-step create: Step 1 details → workflow endpoint → Step 2 editable category/tags prefilled from suggestions → normal create save                                                      |
...
4. ✅ Manual create two-step workflow UI: Step 1 transaction details → workflow endpoint → Step 2 category/tags prepopulated from suggestions and editable before save → normal create save.
