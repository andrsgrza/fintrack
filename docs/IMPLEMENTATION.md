...
#### Domain rules 🟡 (solo ownership hoy)
...
| Admin accede a todo                           | ✅     |                      |
| No borrar si tiene `FinancialTransaction`     | ⏳     | delete guard         |
| `CreditAccountDetails` solo si `CREDIT_CARD`  | ⏳     | cross-entity         |
| Balance actual = f(transacciones) + initial   | ⏳     | read model / service |
| Desactivar cuenta con efectos en presupuestos | ⏳     |                      |
...
| Service — negocio | ✅     | **`currency` / `accountType` immutable** post-create; owner preserve; scoped read/write       |
| REST              | ✅     | `@Valid` POST/PUT; **PATCH JsonNode**; `IllegalArgumentException` → `400 invalid`           |

**Tests:** 108 IT + 11 service — ver [TESTING.md § FA](TESTING.md#financialaccount).
...
| 2026-07-11 | FA DELETE architecture: orchestration in `FinancialAccountService`; delegate `deleteAllForAccount` to `FinancialTransactionService` + `TransactionIngestionService`; single transaction; **blocked** until FT/TI delete rules defined. |
