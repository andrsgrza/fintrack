...
When a FILE workflow is `READY` with valid rows missing candidates, its single review page automatically calls `POST /api/transaction-ingestions/{id}/candidates/prepare` once for the current missing-row set, reloads the workflow read model, validates that every `VALID` row has a usable candidate, then calls `POST /api/transaction-ingestions/{id}/candidates/rule-preview`. It does not prepare candidates for invalid, rejected, or disabled rows, and it avoids preparation on every render. The preview response is transient and keyed by candidate id; it does not mutate candidate category/tags. User category/tag edits call the ingestion-scoped candidate classification PATCH endpoint. Suggestions are applied where they are displayed: **Apply suggested category** in the Category cell sends `scope=CATEGORY`; **Apply suggested tags** in the Tags cell sends `scope=TAGS`; **Confirm no suggestions** remains with Rule details. The toolbar keeps separate, always-visible batch commands for descriptions, categories, and tags; category/tag batch commands omit `candidateIds` and send their respective scope. All explicit category/tag applications use `FILL_EMPTY_ONLY`, preserve manual selections, reload candidate summaries, and reconcile only the scope just applied, so a category application leaves a tag suggestion visible and vice versa. A returned `SKIPPED` row is an independently skipped batch result, not a successful application; the UI retains its preview and shows a warning to review affected rows. Category and tag options are loaded through the owner-scoped `/api/categories/selectable` and `/api/tags/selectable` APIs. They show active values for new selection and explicitly include the row candidate's current inactive historical values, visibly labelled Inactive; those historical values cannot be newly selected. Matched rules, conflicts, and skipped outputs are shown compactly in the same table. Browser refresh reloads persisted candidate category/tag selections through the workflow row `candidate` summary.

The same toolbar exposes four scoped reevaluation actions: **Reevaluate descriptions**, **Reevaluate categories**, **Reevaluate tags**, and **Reevaluate all**. Its separate Apply group always renders **Apply all suggested descriptions**, **Apply all suggested categories**, and **Apply all suggested tags**, disabling each when that domain has no pending change. A preview is not automatically pending work: an identical normalized description or category is informational only, and tag application is pending only when `FILL_EMPTY_ONLY` would add at least one new tag. A valid prepared row also exposes **Reevaluate this row**. Above it, four local V1 settings default to descriptions/categories/tags auto-apply off and protect manual changes on. They are intentionally not persisted: reloading the page restores defaults. Enabling a scope after candidates are ready runs that automatic scope once; a later global or row reevaluation applies only enabled scopes and otherwise remains preview-only. A transient description result visibly offers **Apply suggested description** in the description cell only when it differs from the current normalized description; that per-row command is explicit consent and applies unprotected. **Apply all suggested descriptions** batches only rows for which a current transient change would be applied under the manual-protection setting, preserving protected `USER_EDIT` rows and their suggestion. Description requests use `apply` and `protectManualChanges`; category/tag automatic requests use `automatic`, `protectManualChanges`, and `CATEGORY`/`TAGS`/`ALL` scope. Protected manual values stay selected while the fresh recommendation remains visible; unprotected automation can replace/remove manual values. Automatic category/tag reconciliation never writes `rawData`. If the independent description/classification parts of Reevaluate all do not both succeed, the UI retains the successful domain result and reports partial failure. Confirm Import remains separate.
