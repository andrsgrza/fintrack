...
When a FILE workflow is `READY`, the review page transitions to Pantalla 2 in the same route. Pantalla 2 first prepares/syncs `FILE_IMPORT` candidates with `POST /api/transaction-ingestions/{id}/candidates/prepare`, reloads the workflow read model, validates that every `VALID` row has a usable candidate, then calls `POST /api/transaction-ingestions/{id}/candidates/rule-preview`. The preview response is transient and keyed by candidate id; it does not mutate candidate category/tags. User category/tag edits call the ingestion-scoped candidate classification PATCH endpoint, and per-row rule apply / confirm-no-suggestions actions persist review state on the candidate. Category options are loaded from the existing Category list API and filtered by row flow when practical; Tag options are loaded from the existing Tag list API. Matched rules, conflicts, and skipped outputs are shown compactly. Browser refresh reloads persisted candidate category/tag selections through the workflow row `candidate` summary. Confirm Import still calls the existing `POST /api/transaction-ingestions/{id}/confirm` path. TC-3D.1 backend confirm reads persisted `FILE_IMPORT` candidates as the source of truth, creates `FinancialTransaction` rows from candidate fields/category/tags, keeps `DISABLED` rows skipped, updates the review from the response, and does not persist category/tag choices into `rawData`. The temporary frontend adapter may still submit the legacy confirm payload, but request category/tag ids are not authoritative.
