...
The same toolbar exposes four unambiguous, preview-only reevaluation actions: **Reevaluate descriptions**, **Reevaluate categories**, **Reevaluate tags**, and **Reevaluate all**. A valid prepared row also exposes **Reevaluate this row**. Description reevaluation posts an optional record-id batch to `POST /api/transaction-ingestions/{id}/descriptions/reevaluate`; it evaluates `rawData.raw.description` with the existing normalization evaluator and leaves `USER_EDIT` values untouched, showing any new result only as a transient suggestion. Category/tag reevaluation uses the existing candidate `rule-preview` command with `CATEGORY`, `TAGS`, or `ALL` scope. These actions never call Apply, Confirm no suggestions, or Confirm Import, and category/tag suggestions never go into `rawData`. If the two independent parts of Reevaluate all do not both succeed, the UI retains only the successful transient result and reports the partial failure. **Apply all classification suggestions** is deliberately separate from every reevaluation action; it is an explicit batch persistence command, not reevaluate-and-apply. Per-row Apply suggestions and Confirm no suggestions remain available.
