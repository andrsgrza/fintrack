...
- The workflow E2E also asserts all four global reevaluation controls and the eligible-row action are visible, sends their description/candidate-preview requests with the correct global or row scope, and proves no rule-apply or confirm-no-suggestions request occurs until the test explicitly uses Apply suggestions. It creates a description rule after CSV upload, verifies auto-apply off keeps reevaluated descriptions transient, explicitly applies one suggested description, applies the remaining suggestions in one batch, and verifies neither description action invokes category/tag rule application or Confirm Import.
