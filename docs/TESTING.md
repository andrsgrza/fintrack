...
**Balance calculation status:** calculated balance read models are implemented at `GET /api/financial-accounts/{id}/balance` and batch `GET /api/financial-accounts/overview`. They are calculated on demand, not persisted, and use `transactionDate` from `initialBalanceDate` through `asOfDate` (today for the overview). FinancialAccount detail and overview UI display the results. Charts, dashboard aggregation, interest, statement cycles, and persisted balance recalculation remain deferred/open.
...
`FinancialAccountResourceIT` characterizes the product-only configured endpoints:

- debit creation persists only the scoped account;
- credit-card creation persists the account and exactly one details row in one transaction;
- invalid card details and a non-card payload with card details leave no parent or child write behind;
- configured update changes the existing child, creates a missing historical child only for a credit-card account, preserves account type/currency immutability, and rolls back the parent when child validation fails;
- a current user cannot update another user's account through the configured endpoint.
...
The FinancialAccount UI specs cover the product overview (calculated balances/debt, translated types, status, incomplete cards, empty/error states, and action links) plus dynamic opening-position labels/help text for create/detail views, account-type change reset behavior, preservation of unrelated create-form fields, hidden `createdAt` / `updatedAt` inputs, and disabled `accountType` / `currency` selects in edit mode.
...
- Batch apply with no `candidateIds` covers all candidates in the current ingestion for the supplied scope: characterization verifies multiple eligible candidates receive category/tag outputs, a manual `USER_SELECTED` candidate is preserved, `rawData` is unchanged, and no `FinancialTransaction` is created. Frontend coverage verifies the always-visible global **Apply all suggested categories** and **Apply all suggested tags** actions send `CATEGORY` and `TAGS`, respectively, disable duplicate clicks, reload candidate summaries, preserve manual selections, retain the other domain's transient preview, report independently skipped rows, and do not call reevaluation or Confirm Import.
...
- Description reevaluation tests call `POST /api/transaction-ingestions/{id}/descriptions/reevaluate` for an ingestion or selected rows. They cover active-rule priority ordering, no-match behavior, owner/record scoping, non-VALID skip behavior, preview-only suggestions, protected `USER_EDIT`, explicit unprotected replacement, candidate synchronization when normalized description changes, batch application, and the absence of category/tag writes or FinancialTransaction creation. Frontend coverage distinguishes transient description preview from the per-row **Apply suggested description** command (unprotected explicit consent) and global **Apply all suggested descriptions** (batch application retaining protected `USER_EDIT` rows); an equal current/suggested description is a successful no-op preview and exposes neither apply action, while a protected manual difference remains available only to the per-row explicit action.
...
- Editing a valid row with invalid values renders it as rejected with row error.
- Disabled rows show Enable only; PATCH edit is rejected until the row is enabled.
- Amount and flow are not editable in the row edit UI.
- Save calls `PATCH /api/transaction-ingestions/{ingestionId}/records/{recordId}` with normalized editable fields.
- Cancel leaves the row unchanged and does not call the backend.
- Rejected row error renders in the row table.
- Confirm Import renders only for `READY` reviews with at least one valid row.
- Confirm Import does not render for `PARTIALLY_READY`, zero-valid-row, or completed read-only reviews.
- Confirm success marks imported rows as `IMPORTED`, leaves disabled rows `DISABLED`, updates counts, and makes the review read-only.
- Confirm API error renders a review error and does not mutate the displayed review optimistically.

### I2B.2 resource/integration tests — edit normalized review rows

- Edit `VALID` with valid data -> `VALID`.
- Edit `VALID` with invalid data -> `REJECTED`.
- Edit `REJECTED` with valid data -> `VALID`.
...
- top-level `errorCode` / `errorMessage` update from the first validation error.
- `amount` and `flow` are derived from `signedAmount`; client-provided `amount`, `flow`, and `status` are ignored.
...
- TI-CONFIG-1 coverage proves the same page renders the four local configuration controls with defaults description/category/tag auto-apply off and manual protection on. With all scopes off, reevaluation stays preview-only. Enabling a scope applies only that scope once candidates are ready and on subsequent matching reevaluations; toggling protection alone does not mutate data. Category/tag automatic scope requests carry `automatic=true`, requested scope, and protection; description requests carry `apply` and protection. With description auto-apply off, tests require reevaluate → transient suggestion → explicit one-row apply or global suggested-row batch apply. The per-row path uses `protectManualChanges=false`; the batch path respects the setting. Apply controls and the classification Suggestions metric count only persisted-value diffs: matching description/category previews and already-present suggested tags are no-op previews, not pending work. Classification Apply and Confirm Import remain separate.
...
- The workflow E2E also asserts all four global reevaluation controls and the eligible-row action are visible, sends their description/candidate-preview requests with the correct global or row scope, and proves no rule-apply or confirm-no-suggestions request occurs until the test explicitly applies the relevant domain suggestion. It verifies per-row category/tag apply uses `CATEGORY`/`TAGS`, the global category action leaves tag suggestions pending until the global tag action runs, and description application does not invoke category/tag rule application or Confirm Import.
...
- stale `PARTIALLY_READY` with only valid rows can become `READY` and import.
- not-ready/disallowed statuses are rejected.
- completed ingestion retry is idempotent and creates no duplicates.
- corrupt row/transaction links are rejected defensively.
- completed ingestions reject row review actions.
- foreign ingestion confirm is rejected.
