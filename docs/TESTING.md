...
**Scope:** TC-3C.1 includes manual draft recovery query/UI, backend candidate rule preview/apply commands, manual candidate suggestions UI for MANUAL candidates, backend-only FILE import candidate prepare/sync, optional prepared FILE candidate summaries in the TransactionIngestion workflow response, and backend-only ingestion-scoped FILE candidate classification commands. Pantalla 2 frontend, Confirm Import, DescriptionNormalizationRule re-evaluation, and UserPreference are not migrated to candidates yet.
...
Key TC-3C.1 FILE import candidate classification assertions:
...
**Scope:** canonical CSV import workflow. I1 creates persisted workflows. I2B adds review actions. I2C confirms ready review rows into `FinancialTransaction` rows. TC-3A adds backend-only FILE import candidate prepare/sync. TC-3B exposes optional prepared candidate summaries in the workflow response. TC-3C.1 adds backend-only FILE candidate classification commands. CSV v1 Confirm Import and Pantalla 2 frontend are not migrated to candidates yet.
...
- TC-3C.1 candidate classification endpoints persist category/tags on prepared `FILE_IMPORT` candidates only; they do not persist category/tags into `rawData` and do not create transactions.
- TC-3C.1 candidate preview/apply uses `TransactionOrigin.FILE_IMPORT` and returns candidate-scoped suggestions without using `/api/financial-transactions/rule-preview`.
