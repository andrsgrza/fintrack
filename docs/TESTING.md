...
- Edit action appears for `VALID`, `REJECTED`, and `DISABLED` rows, but not for imported/immutable rows.
...
- Editing a disabled row with valid values re-enables it as valid.
...
- Edit `DISABLED` with valid data -> `VALID`.
- Edit `DISABLED` with invalid data -> `REJECTED`.
...
| 2026-07-18 | CSV Ingestion I2B.2 row edit          | Backend IT covers PATCH normalized row edit for `VALID`/`REJECTED`/`DISABLED`, immutable imported/skipped/failed rows, rawData raw preservation, derived amount/flow, counters/status recalculation, and no `FinancialTransaction` creation. Frontend tests cover inline edit, save/cancel, derived-field read-only behavior, and no confirm/import action. |
