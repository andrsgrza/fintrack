...
- The unified ingestion review uses candidate-backed state. Its local V1 configuration defaults to description/category/tag auto-apply off and manual-change protection on; it is not persisted across page reloads. Enabling an automatic scope after candidates are prepared runs that scope once, and a subsequent scoped/all/row reevaluation applies only the enabled scopes. With every auto scope off, reevaluation stays preview-only. Each domain has its own explicit application action: descriptions use **Apply suggested description** / **Apply all suggested descriptions**; categories use **Apply suggested category** / **Apply all suggested categories** with `scope=CATEGORY`; tags use **Apply suggested tags** / **Apply all suggested tags** with `scope=TAGS`. These actions appear or enable only for a pending state change: exact description/category matches are not applicable, a protected `USER_EDIT` is excluded from the description batch, and a tag suggestion is applicable only when `FILL_EMPTY_ONLY` would add a missing tag. Category/tag actions remain `FILL_EMPTY_ONLY`, and scoped reconciliation retains the other domain's transient suggestion. None of these review commands confirms import, mutates category/tags in `rawData`, or creates a `FinancialTransaction`.
...
During FILE ingestion upload, matched description normalization rules update `rawData.normalized.description` before row review. They do not create transactions and do not invoke category/tag `TransactionRule` evaluation. The same evaluator is available after upload through the ingestion-scoped description reevaluation command; it continues to use the immutable original description and preserves `USER_EDIT` values by default. With auto-apply disabled, reevaluation returns a transient suggestion; the review UI can explicitly apply one suggestion (including replacing a `USER_EDIT` value after that row-level consent) or apply all current suggestions while preserving protected manual descriptions.
