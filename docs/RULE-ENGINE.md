...
- tag assignment;
- resulting description assignment.
...
This is too limiting because one high-priority rule may only set a category while a later matching rule could still add tags or a subscription.
...
The first matching rule with `resultingFinancialSubscription` can suggest a subscription.

If the transaction has no explicit subscription, the subscription can be auto-filled in modes that allow mutation.

If the transaction already has a different explicit subscription, the engine must not silently override it. It should produce a conflict or suggestion.

When applied, subscription output must still respect `FinancialTransaction` domain validation.
...
A matching rule can suggest a replacement or normalized description.

Description handling must be explicit because `FinancialTransaction.description` is usually required.

The engine must not silently override a user-entered description in manual UI flows without confirmation.

In automatic fill-empty mode, `resultingDescription` may only apply if the target description field is considered unset or generated by import.

Final exact behavior remains an open decision before implementation.
...
- Exact `resultingDescription` target behavior.
