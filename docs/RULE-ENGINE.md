...
The unified ingestion review exposes explicit scoped reevaluation controls and a local configuration panel: auto-apply descriptions, categories, tags, and protect manual changes. All three auto-apply controls default off and protection defaults on; this configuration is page-local and is not persisted. With a scope off, reevaluation returns only a suggestion. A description preview now has a separate **Apply suggested description** action beside that description and a batch **Apply all suggested descriptions** action. The first is explicit user consent and sends unprotected description application for that one row; the latter applies only currently suggested rows while respecting manual protection. With automatic scope enabled, the matching reevaluation path persists that scope: category/tag automation synchronizes only its scoped outputs, and description automation uses the existing normalized-description provenance model. A protected manual current value remains selected while its rule recommendation can remain visible; unprotected automatic application may replace or clear it. No automatic review operation calls Confirm Import or creates a FinancialTransaction. Explicit per-row/global category/tag Apply suggestions remains separate `FILL_EMPTY_ONLY` and independent of the configuration.
