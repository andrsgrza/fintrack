...
const apiUrl = 'api/api-access-tokens';

// Actions

export const getEntities = createAsyncThunk(
  'apiAccessToken/fetch_entity_list',
  async ({ sort }: IQueryParams) => {
    const requestUrl = `${apiUrl}?${sort ? `sort=${sort}&` : ''}cacheBuster=${new Date().getTime()}`;
    return axios.get<IApiAccessToken[]>(requestUrl);
  },
  { serializeError: serializeAxiosError },
);

export const getEntity = createAsyncThunk(
  'apiAccessToken/fetch_entity',
...
export const partialUpdateEntity = createAsyncThunk(
  'apiAccessToken/partial_update_entity',
  async (entity: IApiAccessToken, thunkAPI) => {
    const result = await axios.patch<IApiAccessToken>(`${apiUrl}/${entity.id}`, cleanEntity(entity));
    thunkAPI.dispatch(getEntities({}));
    return result;
  },
  { serializeError: serializeAxiosError },
...
export const deleteEntity = createAsyncThunk(
  'apiAccessToken/delete_entity',
  async (id: string | number, thunkAPI) => {
    const requestUrl = `${apiUrl}/${id}`;
    const result = await axios.delete<IApiAccessToken>(requestUrl);
    thunkAPI.dispatch(getEntities({}));
    return result;
  },
  { serializeError: serializeAxiosError },
...
export const ApiAccessTokenSlice = createEntitySlice({
  name: 'apiAccessToken',
  initialState,
...
export const { reset } = ApiAccessTokenSlice.actions;
