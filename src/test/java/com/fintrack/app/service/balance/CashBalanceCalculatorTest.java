...
class CashBalanceCalculatorTest {
    ...
    @Test
    void supportsCashAccounts() {
        assertThat(calculator.supports(AccountType.CASH)).isTrue();
        assertThat(calculator.supports(AccountType.DEBIT)).isFalse();
    }

    @Test
    void calculatesPositiveInitialBalance() {
        var snapshot = calculator.calculate(account(AccountType.CASH, "100.00"), List.of(), Optional.empty(), LocalDate.of(2026, 1, 31));
    ...
    void calculatesZeroInitialBalance() {
        var snapshot = calculator.calculate(account(AccountType.CASH, "0.00"), List.of(), Optional.empty(), LocalDate.of(2026, 1, 31));
    ...
    void calculatesNegativeInitialBalance() {
        var snapshot = calculator.calculate(account(AccountType.CASH, "-50.00"), List.of(), Optional.empty(), LocalDate.of(2026, 1, 31));
    ...
    void inflowIncreasesAndOutflowDecreasesBalance() {
        var snapshot = calculator.calculate(
            account(AccountType.CASH, "100.00"),
            List.of(
